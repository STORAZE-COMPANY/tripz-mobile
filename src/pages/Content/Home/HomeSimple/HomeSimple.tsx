...
import {
  getAllCities,
  getCommercesByCityId,
  getTouristPointsByCityId,
  getEventsByCityId,
  getAllCategories,
} from "@mobile/services/UserService";
...
function HomeSimple() {
  const initialBottomButton = 8; // Altura inicial da caixa
  ...
  const [bottomButton, setBottomButton] = useState(initialBottomButton);

  const [cities, setCities] = useState<
    { name: string; uf: string; id: string }[]
  >([]);
      ...
      if (selectedCity) {
        try {
          ...
          if (response && response.content) {
            ...
            console.error(
              `Nenhum dado encontrado para a cidade: ${selectedCity.name}`
            );
          }
        } catch (error) {
          console.error(
            `Erro ao buscar ${activeTab.toLowerCase()}:`,
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (categoriesData && categoriesData.content) {
          const formattedCategories = categoriesData.content.map(
            (category) => ({
              label: category.name,
              ...
              >
                ...
                >
                  <SvgUri
                    ...
                    stroke="red"
                    strokeOpacity="100%"
                    style={{ marginLeft: -6, marginRight: 4, right: 5 }}
                  />
                  <Text
                    style={{
                      color: "#2D3370",
  ...
  return (
    <Background {...backgroundStyle}>
      <CityHeader
        ...
        coverImage={selectedCity?.coverImage || img_city}
      />
      <SearchInput
        cities={cities}
      ...
      >
        <Box flexDirection="row">
          <TouchableOpacity
            onPress={() => handleTabPress("Pontos Turisticos")}
            style={{ flex: activeTab === "Pontos Turisticos" ? 3 : 3 }}
          >
            <Box
              alignItems="center"
              borderColor={
                activeTab === "Pontos Turisticos"
                  ? lightTheme.colors.primary
                  : "transparent"
              }
              borderBottomWidth={activeTab === "Pontos Turisticos" ? 1.5 : 0}
              pdVertical={7}
            >
              <Text
                style={
          ...
          <TouchableOpacity
            onPress={() => handleTabPress("Comércios")}
            style={{ flex: activeTab === "Comércios" ? 2 : 2 }}
          >
            <Box
              alignItems="center"
              borderColor={
              ...
              borderBottomWidth={activeTab === "Comércios" ? 1.5 : 0}
              pdVertical={7}
            >
              <Text
                style={
                  activeTab === "Comércios" ? textStyles : textDisableStyles
                }
              >
                Comércios
              </Text>
          ...
          <TouchableOpacity
            ...
            style={{ flex: activeTab === "Eventos" ? 2 : 2 }}
          >
            <Box
              alignItems="center"
              borderColor={
              ...
              borderBottomWidth={activeTab === "Eventos" ? 1.5 : 0}
              pdVertical={7}
            >
              <Text
                style={activeTab === "Eventos" ? textStyles : textDisableStyles}
              >
                Eventos
              </Text>
            </Box>
      ...
      <ScrollView
        style={{ flex: 1, marginTop: scale(-60), bottom: 2 }}
      ...
      >
        ...
        >
          <Text
            style={{ fontFamily: "Lato", fontSize: 16, fontWeight: "bold" }}
          >
            Populares
          </Text>
          <TouchableOpacity>
